...
class PipelineManager {
  ...
  static async getInstance(progressCallback, options = {}) {
    ...
    this.#loadingPromise = this.#initializePipeline(progressCallback, {
      modelName,
      task,
      device,
      maxRetries,
  ...
  static async #initializePipeline(progressCallback, config) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(
          `Initializing pipeline (attempt ${attempt}/${maxRetries})...`
        );
        ...
        console.error(
          `Pipeline initialization attempt ${attempt} failed:`,
          error
        );
...
const classify = async (text, options = {}) => {
  ...
  try {
    ...
    const classifier = await PipelineManager.getInstance((progressData) => {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === ACTION_CLASSIFY_TEXT) {
    classify(message.text).then(sendResponse);
